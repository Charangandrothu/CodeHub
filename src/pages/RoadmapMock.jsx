...
import { generateMockRoadmap } from '../utils/mockRoadmapGenerator';
import { ChevronDown, Check, ArrowRight, Play, RefreshCw, Layers, Zap, Trophy, Flame, Calendar, Lock, Unlock, Clock, BarChart2, Brain, MessageSquare, Shield, Settings, Menu, X, Crown, Sparkles, FileText } from 'lucide-react';
...
// --- Preset Goals matching Mock Test Days ---
const GOALS = {
    expert: 15,
    medium: 30,
    beginner: 45
...
// Theme Color Mapping
const TOPIC_COLORS = {
    'service-based': '#8b5cf6', // purple
    'product-based': '#3b82f6', // blue
    'elite-mocks': '#ec4899',    // pink
    'default': '#8b5cf6'
};
                    ...
                    <div className="text-left space-y-1">
                        ...
                        <div className="flex items-center gap-3 text-[10px] font-medium text-zinc-500">
                            <span className={`px-2 py-0.5 rounded-full border flex items-center gap-1.5 transition-all ${isCompleted
                                ...
                                {section.completed}/{section.totalProblems} Tasks
                            </span>
                                                            ...
                                                            <a href={item.link} className="block cursor-pointer">
                                                                ...
                                                                <div className="flex items-center gap-3 mt-3">
                                                                    ...
                                                                    <motion.span
                                                                        ...
                                                                        whileHover={{ x: 2 }}
                                                                    >
                                                                        {item.type === 'test' ? 'Launch Mock' : 'Go to Page'} <ArrowRight size={10} />
                                                                    </motion.span>
                                                                </div>
                                                            </a>
                                                        </div>
                ...
                >
                    ...
                    <div className={isLocked ? "opacity-30 grayscale cursor-not-allowed pointer-events-none transition-all duration-500" : "transition-all duration-500"}>
                        ...
                        <div className="flex justify-between items-center px-1 mt-2 min-h-[1.25rem]">
                            ...
                            >
                                {days === 45 && "Recommended for Beginner"}
                                {days === 30 && "Recommended for Medium"}
                                {days === 15 && "Recommended for Expert"}
                                {![15, 30, 45].includes(days) && "Custom Duration"}
                            </motion.span>
                        </div>
                    </div>
...
const RoadmapMock = ({ onBack }) => {
    ...
    const [expandedSection, setExpandedSection] = useState("service-based");
    ...
    const fetchMockTests = async () => {
        try {
            ...
            console.error("Failed to fetch mock tests list:", err);
        }
        return [];
    };
    ...
    const saveRoadmap = async (newRoadmap) => {
        ...
        if (currentUser) {
            try {
                const res = await fetch(`${API_URL}/api/users/roadmap-mock/${currentUser.uid}`, {
                    method: 'PUT',
                ...
                console.error("Failed to sync mock tests roadmap:", err);
            }
        }
    };
    ...
    useEffect(() => {
        ...
        const initRoadmap = async () => {
            ...
            if (userData.mockRoadmap) {
                setRoadmap(userData.mockRoadmap);
                if (userData.mockRoadmap.daysSelected) {
                    setDays(userData.mockRoadmap.daysSelected);
                    const matchedGoal = Object.keys(GOALS).find(key => GOALS[key] === userData.mockRoadmap.daysSelected);
                    setSelectedGoal(matchedGoal || null);
                }
                setLoadingRoadmap(false);
            } else {
                await handleGenerate(30, false, tests);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            handleGenerate(days, true, mockTestsList);
        }, 600);

        return () => clearTimeout(timer);
    }, [days, mockTestsList]);

    const handleGenerate = async (selectedDays, mergeProgress = false, activeTests = mockTestsList) => {
        setIsGenerating(true);
        try {
            const baseRoadmap = generateMockRoadmap(selectedDays, activeTests);
    ...
    const getLevelInfo = (d) => {
        if (d <= 15) return { label: 'Mock crash course', color: 'rose', icon: <Flame size={16} /> };
        if (d <= 30) return { label: 'Mock boot camp', color: 'amber', icon: <Zap size={16} /> };
        return { label: 'Mock full simulation', color: 'emerald', icon: <Trophy size={16} /> };
    };
                    ...
                    >
                        <div className="relative z-10 w-full sm:w-auto text-left ml-0 sm:-ml-14">
                            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-zinc-200 to-emerald-400 mb-2 block pb-1">
                                Mock Tests Curriculum
                            </h1>
                            <p className="text-zinc-500 text-sm max-w-md leading-relaxed">
                                Hone your coding and cognitive speed with structured real-world placement simulators.
                            </p>
                        </div>
                            ...
                            {!roadmap?.isLocked ? (
                                <>
                                    ...
                                    <div className="flex flex-wrap justify-end items-center gap-2 sm:gap-4">
                                        {[
                                            { label: 'Beginner', goal: 'beginner', tests: 'Full Prep', color: 'text-purple-400', border: 'border-purple-500/20' },
                                            { label: 'Medium', goal: 'medium', tests: 'Standard', color: 'text-amber-400', border: 'border-amber-500/20' },
                                            { label: 'Expert', goal: 'expert', tests: 'Sprint', color: 'text-rose-400', border: 'border-rose-500/20' }
                                        ].map((preset) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-1 relative z-10">
                                                        ...
                                                        <span className="text-[9px] font-medium text-zinc-500">({preset.tests})</span>
                                                    </div>
                                                </button>
                                            );
                                        })}
