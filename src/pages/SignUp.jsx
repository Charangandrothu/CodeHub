...
const SignUp = () => {
  ...
  const handleSignup = async (e) => {
    e.preventDefault();
    // Clear previous errors and validate password length before proceeding
    setPasswordError('')
    ...
    try {
      ...
      // 2. Update Auth Profile (Display Name) - NON-BLOCKING
      updateProfile(user, {
        displayName: name,
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#0a0a0a] px-4 sm:px-6 lg:px-8 pt-20">
      ...
      <div className="relative w-full max-w-[350px] z-10 perspective-1000">
        ...
        >
          ...
          <form onSubmit={handleSignup} className="space-y-4 relative z-10">
            ...
            >
              <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase tracking-wider ml-1">Full Name</label>
              <div className="relative">
                <div className={`absolute inset-0 rounded-xl transition-opacity duration-300 ${focusedField === 'name' ? 'opacity-100' : 'opacity-0'}`}>
            ...
            {/* Signup Button with Sweep Effect */}
            <motion.div
            ...
            >
              <button
                ...
                className="group relative w-full flex items-center justify-center gap-2 bg-white text-black font-semibold py-3.5 rounded-xl transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] overflow-hidden cursor-pointer active:cursor-grabbing disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {/* Sweep Gradient */}
      ...
      {/* Add custom keyframe for sweep animation in a style tag */}
      <style>{`
        @keyframes sweep {
