...
const Dashboard = () => {
    ...
    return (
        <div className="min-h-screen bg-app-bg pt-24 pb-12 px-4 sm:px-6 lg:px-8">
            <motion.div
            ...
            >
                ...
                <motion.div variants={itemVariants} className="w-full">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl border border-white/5 bg-app-panel backdrop-blur-xl">
                        <div>
                            ...
                            <p className="text-app-muted text-sm mt-1">Continue your structured placement preparation</p>
                        </div>
                                ...
                                {myWeeklyStats.rank ? (
                                    <>
                                        <div className="flex items-end justify-end gap-2">
                                            <span className="text-2xl font-bold text-app-text">#{myWeeklyStats.rank}</span>
                                    ...
                                    <>
                                        <div className="flex items-end justify-end gap-2">
                                            <span className="text-sm font-bold text-app-muted">Unranked 😴</span>
                                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <motion.div
                            ...
                            className={`group relative overflow-hidden rounded-2xl border ${dashboardState.isNewUser ? 'border-purple-500/30' : 'border-app-border'} bg-gradient-to-br from-blue-900/10 via-white/[0.02] to-transparent p-6 hover:border-blue-500/30 transition-all duration-300`}
                        >
                            ...
                            <div className="relative z-10 flex flex-col gap-6">
                                <div className="flex flex-col sm:flex-row w-full items-start sm:items-center justify-between gap-6">
                                    <div className="flex items-start gap-4">
                                        <div className={`p-3.5 rounded-xl ${dashboardState.isNewUser ? 'bg-gradient-to-br from-purple-600 to-blue-600' : 'bg-gradient-to-br from-blue-600 to-blue-700'} text-app-text shadow-lg shadow-blue-500/20 shrink-0`}>
                                            <Code2 size={24} />
                                        </div>
                                        <div className="space-y-1">
                                            {dashboardState.isNewUser ? (
                                                <>
                                                    <h3 className="text-xl font-bold text-app-text">Start Learning</h3>
                                                    <p className="text-sm text-app-muted mb-3">Begin your placement journey with structured patterns</p>
                                                    <div className="flex flex-wrap gap-2">
                                                        {dashboardState.roadmapPreview.map((tag, i) => (
                                                            <span key={i} className="text-[10px] px-2 py-1 rounded-md bg-app-primary-soft border border-app-border text-app-muted">
                                                                {tag}
                                                            </span>
                                                        ))}
                                                    </div>
                                                ...
                                                <>
                                                    ...
                                                    <h3 className="text-lg font-bold text-app-text mb-1 group-hover:text-blue-300 transition-colors">
                                                        {nextTask?.topic || "Loading..."}
                                                    </h3>
                                                    <div className="flex items-center gap-2 text-sm text-app-muted font-medium mt-1 mb-2">
                    ...
                    <motion.div variants={itemVariants} className="space-y-6">
                        ...
                        {/* Compact Stats Panel */}
                        <div className="rounded-2xl border border-app-border bg-[#0a0a0a]/50 backdrop-blur-xl p-5">
                            <h3 className="text-sm font-semibold text-app-muted uppercase tracking-wider mb-4">You at a glance</h3>
                            ...
                            <div className="mt-6 pt-5 border-t border-white/5">
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="text-app-text font-mono font-medium">{userData?.stats?.globalRank ? `#${userData.stats.globalRank}` : 'Unranked'}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Top Solvers Widget */}
                        <div className="rounded-2xl border border-app-border bg-[#0a0a0a]/50 backdrop-blur-xl p-5">
                            <h3 className="text-sm font-semibold text-app-muted uppercase tracking-wider mb-4 flex items-center gap-2">
                            ...
                            <div className="space-y-3">
                                {weeklyLeaderboard.slice(0, 3).map((user, idx) => (
                                    <div
                                        ...
                                        className="flex items-center justify-between p-2 rounded-lg hover:bg-app-primary-soft cursor-pointer transition-colors group"
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shadow-lg ${idx === 0 ? "bg-gradient-to-br from-yellow-400 to-amber-600 text-app-text shadow-yellow-500/20" :
                                                idx === 1 ? "bg-gradient-to-br from-gray-300 to-gray-500 text-app-text shadow-gray-500/20" :
                                                    "bg-gradient-to-br from-orange-400 to-red-500 text-app-text shadow-orange-500/20"
                                                }`}>
                                                {idx + 1}
                                            </div>
                                            <div>
                                                <p className="text-sm font-medium text-app-text max-w-[120px] truncate group-hover:text-blue-400 transition-colors">
                                                    {user.username || "User"}
                                                </p>
...
const DashboardCard = ({ title, icon: Icon, color, stats, percentage, onClick }) => {
    ...
    return (
        <div
            onClick={onClick}
            className="group p-4 rounded-xl border border-white/5 bg-app-panel hover:bg-white/[0.04] hover:border-app-border transition-all duration-300 cursor-pointer"
        >
            <div className="flex justify-between items-start mb-3">
                <div className={`p-2 rounded-lg bg-app-primary-soft ${colors[color]} transition-colors`}>
                    <Icon size={18} />
                </div>
                <span className="text-xs font-mono text-gray-500 group-hover:text-gray-300 transition-colors">{percentage}%</span>
            </div>

            <h4 className="text-app-text font-medium text-sm mb-1">{title}</h4>
            <p className="text-xs text-gray-500 mb-3">{stats}</p>

            <div className="w-full h-1 bg-app-primary-soft rounded-full overflow-hidden">
                <motion.div
                    initial={{ width: 0 }}
...
const StatRow = ({ icon: Icon, color, bg, label, value }) => (
    <div className="flex items-center justify-between group">
        <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-app-muted group-hover:text-gray-300 transition-colors">{label}</span>
        </div>
        <span className="text-app-text font-medium text-sm font-mono">{value}</span>
    </div>
);
